 ...
 */
function sanitizeObject(obj: any, fieldConfig: Record<string, string>): any {
    ...
    if (typeof obj === 'object') {
        ...
        for (const [key, value] of Object.entries(obj)) {
            ...
            if (typeof value === 'string') {
                sanitized[key] = sanitizeString(value, options ?? DEFAULT_OPTIONS.text as SanitizationOptions);
            } else {
 ...
 */
export const createSanitizationMiddleware = (fieldConfig: Record<string, string>) => {
    return (req: Request, res: Response, next: NextFunction): void => {
        try {
            ...
            if (req.query && typeof req.query === 'object') {
                req.query = sanitizeObject(req.query, fieldConfig);
            }

            // Sanitize URL parameters
            if (req.params && typeof req.params === 'object') {
                req.params = sanitizeObject(req.params, fieldConfig);
            }
 ...
 */
export const sanitizeValue = (value: string, type: keyof typeof DEFAULT_OPTIONS = 'text'): string => {
